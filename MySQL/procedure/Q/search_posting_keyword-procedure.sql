		...
		from(	(select posting.*
						from	(  (select to_user as creator_name # select my friends' friends excluding me
									...
									where (from_user in (select to_user
														 ...
														 select from_user
														 from relation
														 where to_user = search_username)) # one person in relationship should be one of my direct friends
									and
									(to_user not in (select to_user
													 ...
													 select from_user
													 from relation
													 where to_user = search_username)) # the other one should be someone who is not a friend of mine
						...
						where FOF_result.creator_name = posting.creator_name 
							  and P_time >= @starttime_timestamp
							  and post_view_priority>=2 and textfile like (@keyword) )# seaching the specific keywords;
						...
						(select *
						 ...
						 where (creator_name in (select to_user
							   ...
							   and textfile like (@keyword) # seaching the specific keywords
							   and P_time >= @starttime_timestamp
							   and post_view_priority >= 1 )	)  as all_search_result# the view_priority should allow at least their friends to see
		order by P_time desc;
		...
		from(	(select *
				...
				(select posting.*
				 ...
				 where creator_name not in( select *
											from ( (select to_user as creator_name # select my friends' friends and me
													from relation
													where (from_user in (select to_user
																		 from relation
																		 where from_user = search_username
					  ...
					  and P_time >= @starttime_timestamp
					  and textfile like (@keyword) )	) as all_search_result
	order by P_time desc;
